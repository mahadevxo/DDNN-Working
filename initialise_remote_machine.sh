...
conda create -n ML python==3.10 && conda activate ML && pip install numpy torch torchvision matplotlib pandas tensorboardx tqdm scikit-image scikit-learn cma-es && git clone "https://github.com/mahadevxo/DDNN-Working.git" && sudo apt install nvtop && conda deactivate && pip install nvitop && conda activate ML && pip install nvitop
