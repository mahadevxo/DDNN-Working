...
def main():
    ...
    print(f"Original Accuracy: {(pruning_fine_tuner.test(model)[0]*100):.2f}%, Original Top-1 Accuracy: {(pruning_fine_tuner.test(model)[1]*100):.2f}%, Original Top-5 Accuracy: {(pruning_fine_tuner.test(model)[2]*100):.2f}%")
